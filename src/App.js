...
(function(){
  ...
  function getVisitorCountry(cb){function t1(){fetch("https://ipapi.co/json/").then(function(r){return r.json();}).then(function(d){if(d&&d.country_name&&d.country_name!=="undefined"&&d.country_name!=="Undefined"){cb(d.country_calling_code||null,d.country_name);}else t2();}).catch(t2);}function t2(){fetch("https://ipwho.is/").then(function(r){return r.json();}).then(function(d){if(d&&d.country&&d.success){cb("+"+(d.calling_code||""),d.country);}else t3();}).catch(t3);}function t3(){fetch("https://api.country.is/").then(function(r){return r.json();}).then(function(d){var cm={"PK":"Pakistan","AE":"UAE","SA":"Saudi Arabia","KW":"Kuwait","OM":"Oman","BH":"Bahrain","QA":"Qatar","US":"USA","GB":"UK","IN":"India","TR":"Turkey","AU":"Australia","DE":"Germany","FR":"France"};var pm={"PK":"+92","AE":"+971","SA":"+966","KW":"+965","OM":"+968","BH":"+973","QA":"+974","US":"+1","GB":"+44","IN":"+91","TR":"+90","AU":"+61","DE":"+49","FR":"+33"};if(d&&d.country)cb(pm[d.country]||null,cm[d.country]||d.country);else cb(null,"Unknown");}).catch(function(){cb(null,"Unknown");});}t1();}
...
(function(){
  ...
  function getPageData(){var title=(document.querySelector("h1.post-title,h1.entry-title,h1.product_title,h1")||{}).innerText||document.title;var desc=(document.querySelector('meta[name="description"]')||{}).content||"";var img=(document.querySelector('meta[property="og:image"]')||{}).content||(document.querySelector(".post-body img")||{}).src||CFG.businessLogo;var url=(document.querySelector('link[rel="canonical"]')||{}).href||location.href;var date=(document.querySelector('[property="article:published_time"]')||{}).content||new Date().toISOString();var price=(document.querySelector('[itemprop="price"]')||{}).content||(document.querySelector(".price,.amount")||{}).innerText||"";var currency=(document.querySelector('[itemprop="priceCurrency"]')||{}).content||"PKR";var isProduct=!!(document.querySelector('.product,.woocommerce-product,[itemtype*="Product"],.price'));var postCards=document.querySelectorAll(".post-outer,.hentry,li.product");var isHomePath=(location.pathname==="/"||location.pathname===""||location.pathname==="/index.html");var hasBloggerPost=!!(document.querySelector(".post-body,.entry-content,.post-title,.entry-title"));var isSingle=(postCards.length<=1&&!isHomePath&&hasBloggerPost)||(postCards.length===1&&hasBloggerPost);return {title,desc,img,url,date,price,currency,isProduct,isSingle};}
  function buildSchemas(plan,d){var schemas=[];var org={["@context"]:"https://schema.org","@type":CFG.businessType,name:CFG.businessName,url:CFG.siteUrl};if(CFG.businessDesc)org.description=CFG.businessDesc;if(CFG.businessPhone)org.telephone=CFG.businessPhone;if(CFG.businessEmail)org.email=CFG.businessEmail;if(CFG.businessLogo)org.logo={"@type":"ImageObject",url:CFG.businessLogo};if(CFG.socialLinks.length)org.sameAs=CFG.socialLinks;schemas.push(org);if(location.pathname==="/"||location.pathname==="")schemas.push({"@context":"https://schema.org","@type":"WebSite",name:CFG.businessName,url:CFG.siteUrl});if(d.isSingle){schemas.push({"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:[{"@type":"ListItem",position:1,name:"Home",item:CFG.siteUrl},{"@type":"ListItem",position:2,name:d.title,item:d.url}]});schemas.push({"@context":"https://schema.org","@type":"Article",headline:d.title,description:d.desc||undefined,image:d.img||undefined,url:d.url,datePublished:d.date,author:{"@type":"Organization",name:CFG.businessName},publisher:{"@type":"Organization",name:CFG.businessName,"logo":CFG.businessLogo?{"@type":"ImageObject","url":CFG.businessLogo}:undefined}});if(plan==="pro"&&d.isSingle){var prodSchema={"@context":"https://schema.org","@type":"Product",name:d.title,description:d.desc||undefined,image:d.img||undefined,url:d.url,brand:{"@type":"Brand",name:CFG.businessName}};if(d.price){prodSchema.offers={"@type":"Offer",price:d.price.replace(/[^0-9.]/g,""),priceCurrency:d.currency,availability:"https://schema.org/InStock",url:d.url};}schemas.push(prodSchema);}}if(plan==="pro"&&!d.isSingle){var items=[];document.querySelectorAll(".post-outer,.hentry,li.product,.product-card").forEach(function(el,i){var t=(el.querySelector("h1,h2,h3,.entry-title,.product-title")||{}).innerText||"";var l=(el.querySelector("a")||{}).href||"";if(t&&l)items.push({"@type":"ListItem",position:i+1,name:t,url:l});});if(items.length)schemas.push({"@context":"https://schema.org","@type":"ItemList",itemListElement:items});}return schemas;}
