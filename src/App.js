...
const SESSION_KEY = "wbm_v13_session";
...
const sb = {
  ...
  upsertSite: (s) => sb.query("POST", "sites?on_conflict=id", {
    id: s.id, name: s.name, url: s.url, enabled: s.enabled, payment: s.payment,
    numbers: s.numbers, secret_key: s.secretKey || "", installed_at: s.installedAt,
    last_active: s.lastActive, clicks: s.clicks || 0, impressions: s.impressions || 0,
    verified: s.verified || false, plan: s.plan || "basic",
  }),
  ...
  upsertSchemaSite: (s) => sb.query("POST", "schema_sites?on_conflict=id", {
    id: s.id, name: s.name, url: s.url, enabled: s.enabled, payment: s.payment,
    plan: s.plan || "basic", business_name: s.businessName || "", business_type: s.businessType || "Organization",
    business_desc: s.businessDesc || "", business_phone: s.businessPhone || "",
    business_email: s.businessEmail || "", business_address: s.businessAddress || "",
    business_logo: s.businessLogo || "", social_links: s.socialLinks || [],
    created_at: s.createdAt, verified: s.verified || false,
  }),
...
function genWBMScript(site) {
  ...
  return `<!-- WBManager Geo | ${site.name} | ${site.plan === "pro" ? "PRO" : "BASIC"} | v13 -->
<script>
(function(){
  var CFG={siteId:"${site.id}",numbers:${nums},key:${secret},siteUrl:"${site.url}",plan:"${site.plan||"basic"}",supabaseUrl:"${SUPABASE_URL}",supabaseKey:"${SUPABASE_KEY}"};
  ...
  function getVisitorCountry(cb){fetch("https://ipapi.co/json/",{signal:AbortSignal.timeout(3000)}).then(function(r){return r.json();}).then(function(d){cb(d.country_calling_code||null);}).catch(function(){cb(null);});}
  ...
  function getPrice(){var m=document.querySelector('[property="product:price:amount"],[itemprop="price"]');if(m){var c=document.querySelector('[property="product:price:currency"],[itemprop="priceCurrency"]');return ((c?c.content||c.innerText:"")||"")+" "+(m.content||m.innerText||"").replace(/[^0-9.,]/g,"");}var b=document.body.innerText||"";var ps=[/Price[:\\s]+PKR[\\s]?([\\d,]+)/i,/PKR[\\s]?([\\d,]+)/i,/Rs\\.?[\\s]?([\\d,]+)/i,/\\$\\s?([\\d,]+\\.?\\d{0,2})/,/£\\s?([\\d,]+\\.?\\d{0,2})/,/€\\s?([\\d,]+)/,/₹\\s?([\\d,]+)/,/AED[\\s]?([\\d,]+)/i,/SAR[\\s]?([\\d,]+)/i];var ss=["PKR ","PKR ","Rs. ","$ ","£ ","€ ","₹ ","AED ","SAR "];for(var i=0;i<ps.length;i++){var mx=b.match(ps[i]);if(mx&&mx[1])return (ss[i]||"")+mx[1];}return "";}
  function getImage(){var og=document.querySelector('meta[property="og:image"]');if(og&&og.content)return og.content.replace(/\\/w\\d+-h\\d+(-[^/]*)?(?=\\/|$)/,"");var img=document.querySelector(".post-body img,.entry-content img,.product img,article img");if(img)return (img.src||img.getAttribute("data-src")||"").replace(/\\/w\\d+-h\\d+(-[^/]*)?(?=\\/|$)/,"");return "";}
  ...
  function addFAB(number,plan){if(document.getElementById("wbm-fab"))return;var msg=buildMsg(plan);var wrap=document.createElement("div");wrap.id="wbm-fab";var a=document.createElement("a");a.href="https://wa.me/"+number.replace(/\\D/g,"")+"?text="+encodeURIComponent(msg);a.target="_blank";a.rel="noopener noreferrer";a.innerHTML=WA_SVG;wrap.appendChild(a);document.body.appendChild(wrap);}
  function removeFAB(){var el=document.getElementById("wbm-fab");if(el)el.remove();}
  function init(){checkStatus(function(active,plan){if(!active){removeFAB();return;}getVisitorCountry(function(code){var number=pickNumber(code);addFAB(number,plan);});});setInterval(function(){checkStatus(function(active,plan){removeFAB();if(!active)return;getVisitorCountry(function(code){addFAB(pickNumber(code),plan);});});},5*60*1000);}
...
function genSchemaScript(site) {
  ...
  return `<!-- Schema Markup Manager | ${site.name} | ${isPro ? "PRO" : "BASIC"} | v13 -->
<script>
(function(){
  var CFG={
    siteId:"${site.id}",
    ...
    businessType:"${site.businessType || "Organization"}",
    businessDesc:"${site.businessDesc || ""}",
    businessPhone:"${site.businessPhone || ""}",
    businessEmail:"${site.businessEmail || ""}",
    businessAddress:"${site.businessAddress || ""}",
    businessLogo:"${site.businessLogo || ""}",
  ...
  function injectSchema(schema){
    var existing=document.querySelector('script[data-wbm-schema="'+schema["@type"]+'"]');
    if(existing)existing.remove();
    var el=document.createElement("script");
    el.type="application/ld+json";
    el.setAttribute("data-wbm-schema",schema["@type"]);
    el.textContent=JSON.stringify(schema);
    document.head.appendChild(el);
  }

  function getPageData(){
    var title=(document.querySelector("h1.post-title,h1.entry-title,h1.product_title,h1")||{}).innerText||document.title;
    var desc=(document.querySelector('meta[name="description"]')||{}).content||(document.querySelector(".post-body p,.entry-content p")||{}).innerText||"";
    var img=(document.querySelector('meta[property="og:image"]')||{}).content||(document.querySelector(".post-body img,.entry-content img")||{}).src||CFG.businessLogo;
    var url=(document.querySelector('link[rel="canonical"]')||{}).href||location.href;
    var date=(document.querySelector('[property="article:published_time"],[itemprop="datePublished"]')||{}).content||new Date().toISOString();
    var modified=(document.querySelector('[property="article:modified_time"],[itemprop="dateModified"]')||{}).content||date;
    var price=(document.querySelector('[itemprop="price"],[property="product:price:amount"]')||{}).content||(document.querySelector(".price,.amount,.product-price")||{}).innerText||"";
    var currency=(document.querySelector('[itemprop="priceCurrency"],[property="product:price:currency"]')||{}).content||"PKR";
    var isProduct=!!(document.querySelector('.product,.woocommerce-product,[itemtype*="Product"],.price,.amount'));
    var isSingle=document.querySelectorAll(".post-outer,.hentry,li.product").length<=1&&location.pathname!=="/"&&location.pathname!=="";
    return {title,desc,img,url,date,modified,price,currency,isProduct,isSingle};
  }

  function buildSchemas(plan,pageData){
    var schemas=[];
    var {title,desc,img,url,date,modified,price,currency,isProduct,isSingle}=pageData;

    // 1. Organization/LocalBusiness — ALL pages, ALL plans
    var orgSchema={
      "@context":"https://schema.org",
      "@type":CFG.businessType,
      "name":CFG.businessName,
      "url":CFG.siteUrl,
      "description":CFG.businessDesc||undefined,
      "telephone":CFG.businessPhone||undefined,
      "email":CFG.businessEmail||undefined,
      "address":CFG.businessAddress?{"@type":"PostalAddress","streetAddress":CFG.businessAddress}:undefined,
      "logo":CFG.businessLogo?{"@type":"ImageObject","url":CFG.businessLogo}:undefined,
      "sameAs":CFG.socialLinks.length?CFG.socialLinks:undefined,
    };
    Object.keys(orgSchema).forEach(function(k){if(orgSchema[k]===undefined)delete orgSchema[k];});
    schemas.push(orgSchema);

    // 2. WebSite — ALL plans, home page
    if(location.pathname==="/"||location.pathname===""){
      schemas.push({
        ...
        "name":CFG.businessName,
        "url":CFG.siteUrl,
        "potentialAction":{"@type":"SearchAction","target":CFG.siteUrl+"/?q={search_term_string}","query-input":"required name=search_term_string"}
      });
    }

    // 3. BreadcrumbList — ALL plans, single pages
    if(isSingle){
      schemas.push({
        "@context":"https://schema.org",
        "@type":"BreadcrumbList",
    ...
    // 4. Article Schema — BASIC + PRO, single blog post
    if(isSingle&&!isProduct){
      schemas.push({
        ...
        "author":{"@type":"Organization","name":CFG.businessName},
        "publisher":{"@type":"Organization","name":CFG.businessName,"logo":CFG.businessLogo?{"@type":"ImageObject","url":CFG.businessLogo}:undefined}
      });
    }

    // 5. Product Schema — PRO only, product pages
    if(plan==="pro"&&isSingle&&isProduct&&price){
      schemas.push({
        "@context":"https://schema.org",
        "@type":"Product",
        "name":title,
        ...
        "brand":{"@type":"Brand","name":CFG.businessName}
      });
    }

    // 6. ItemList — PRO only, listing/home pages with products
    if(plan==="pro"&&!isSingle){
      var items=[];
      document.querySelectorAll(".post-outer,.hentry,li.product,.product-card,.product").forEach(function(el,i){
        var t=(el.querySelector("h1,h2,h3,.entry-title,.product-title")||{}).innerText||"";
        var l=(el.querySelector("a")||{}).href||"";
        if(t&&l)items.push({"@type":"ListItem","position":i+1,"name":t,"url":l});
      });
      if(items.length){
        schemas.push({"@context":"https://schema.org","@type":"ItemList","itemListElement":items});
      }
    }
  ...
  function init(){
    checkStatus(function(active,plan){
      ...
      var schemas=buildSchemas(plan,pageData);
      schemas.forEach(function(s){injectSchema(s);});
    });
    // Recheck every 5 min
    setInterval(function(){
      checkStatus(function(active,plan){
        document.querySelectorAll('script[data-wbm-schema]').forEach(function(el){el.remove();});
        if(!active)return;
        buildSchemas(plan,getPageData()).forEach(function(s){injectSchema(s);});
      });
    },5*60*1000);
    // Watch for dynamic page changes (SPA/Blogger infinite scroll)
...
.btn-schema{background:linear-gradient(135deg,#7c3aed,#6d28d9);color:#fff;border:none;border-radius:9px;padding:10px 18px;font-weight:700;font-size:13px;cursor:pointer;white-space:nowrap;box-shadow:0 2px 8px rgba(124,58,237,.3);}
...
.inp-schema:focus{border-color:#7c3aed;box-shadow:0 0 0 3px rgba(124,58,237,.1);}
.nav-section{font-size:10px;font-weight:700;color:#94a3b8;letter-spacing:1px;text-transform:uppercase;padding:12px 14px 6px;}
...
.nav-btn:hover{background:#f8fafc;color:#475569;}
...
.tab-btn.active-schema{background:#fdf4ff;color:#7c3aed;font-weight:700;}
...
.spinner-schema{border-top-color:#7c3aed;}
...
.schema-badge{background:#fdf4ff;color:#7c3aed;font-size:10px;font-weight:700;padding:2px 8px;border-radius:20px;border:1px solid #e9d5ff;}
...
@media(max-width:899px){.stats-grid{grid-template-columns:repeat(3,1fr);}.form-grid{grid-template-columns:1fr;}.page{padding:14px;}.plan-compare{grid-template-columns:1fr;}}
      ...
      <div className="login-card">
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ display: "flex", justifyContent: "center", gap: 8, marginBottom: 12 }}>
            ...
            <div style={{ width: 44, height: 44, background: "#fdf4ff", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>🔖</div>
          </div>
          <div style={{ fontSize: 22, fontWeight: 800, color: "#1e293b" }}>WBManager Suite</div>
          <div style={{ display: "flex", gap: 6, justifyContent: "center", marginTop: 8 }}>
            <span style={{ background: "#dcfce7", color: "#16a34a", fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20, border: "1px solid #86efac" }}>WhatsApp Manager</span>
            <span style={{ background: "#fdf4ff", color: "#7c3aed", fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20, border: "1px solid #e9d5ff" }}>Schema Manager</span>
          </div>
        </div>
        ...
        {mode === "recover" && (<>
          <div style={{ background: "#fef9c3", border: "1px solid #fde68a", borderRadius: 10, padding: "12px 14px", marginBottom: 16, fontSize: 13, color: "#854d0e" }}>🔑 Recovery: <b style={{ fontFamily: "monospace" }}>WBM-RECOVERY-2026-ADNAN</b></div>
  ...
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
      {Object.values(groups).map(g => (
        <span key={g.info.code} style={{ background: "#fdf4ff", border: "1px solid #e9d5ff", borderRadius: 20, padding: "3px 10px", fontSize: 11, color: "#6b21a8", fontWeight: 600 }}>
          {g.info.flag} {g.info.name} ({g.nums.length})
        </span>
      ))}
    </div>
  );
    ...
    <div className="site-card">
      ...
      {isSchema && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
          <span className="schema-type-badge">Organization</span>
          <span className="schema-type-badge">Article</span>
          <span className="schema-type-badge">BreadcrumbList</span>
          <span className="schema-type-badge">WebSite</span>
          {site.plan === "pro" && <><span className="schema-type-badge">Product</span><span className="schema-type-badge">ItemList</span></>}
        </div>
      )}

      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        {[`📅 ${fmtDate(site.installedAt || site.createdAt)}`, `🕐 ${timeAgo(site.installedAt || site.createdAt)}`].map(c => <span key={c} className="chip">{c}</span>)}
      </div>
...
export default function App() {
  ...
  const [activeModule, setActiveModule] = useState("wbm"); // wbm | schema
  ...
  useEffect(() => { const interval = setInterval(() => { if (loggedIn && !getSession()) setLoggedIn(false); if (loggedIn) setSession(); }, 60000); return () => clearInterval(interval); }, [loggedIn]);
  ...
  useEffect(() => { if (loggedIn) { loadWBM(); loadSchema(); } }, [loggedIn, loadWBM, loadSchema]);
  ...
  async function saveWBM() {
    ...
    if (res !== null) { toast_(viewWBM === "add" ? "Website add ho gayi! ✅" : "Update ho gayi! ✅"); await loadWBM(); goWBM("dashboard"); } else toast_("Error!", "err");
    setSaving(false);
  }

  async function toggleWBM(site) { if (site.payment === "pending") { toast_("Pehle payment complete karein!", "err"); return; } const ne = !site.enabled; setSites(p => p.map(s => s.id === site.id ? { ...s, enabled: ne } : s)); await sb.updateSite(site.id, { enabled: ne }); toast_(ne ? "✅ Button ON!" : "⏸ Button OFF!"); }
  ...
  async function deleteWBM(id) { await sb.deleteSite(id); setSites(p => p.filter(s => s.id !== id)); setDelItem(null); toast_("Remove ho gayi!", "err"); }
  ...
  async function saveSchema() {
    ...
    if (res !== null) { toast_(viewSchema === "add" ? "Schema site add ho gayi! ✅" : "Update ho gayi! ✅"); await loadSchema(); goSchema("dashboard"); } else toast_("Error!", "err");
    setSaving(false);
  }

  async function toggleSchema(site) { if (site.payment === "pending") { toast_("Pehle payment complete karein!", "err"); return; } const ne = !site.enabled; setSchemaSites(p => p.map(s => s.id === site.id ? { ...s, enabled: ne } : s)); await sb.updateSchemaSite(site.id, { enabled: ne }); toast_(ne ? "✅ Schema ON!" : "⏸ Schema OFF!"); }
  ...
  async function deleteSchema(id) { await sb.deleteSchemaSite(id); setSchemaSites(p => p.filter(s => s.id !== id)); setDelItem(null); toast_("Remove ho gayi!", "err"); }

  const toastBg = { ok: "#16a34a", err: "#dc2626", warn: "#d97706" };
  const isSchemaModule = activeModule === "schema";
  const currentView = isSchemaModule ? viewSchema : viewWBM;
  ...
  return (
    <div className="wbm-root">
      ...
      <aside className={`sidebar${!isWide && !navOpen ? " hidden" : ""}`}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "16px 14px", borderBottom: "1px solid #e2e8f0" }}>
          <div style={{ display: "flex", gap: 4 }}>
            ...
            <div style={{ width: 32, height: 32, background: "#fdf4ff", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>🔖</div>
          </div>
          <div>
            <div style={{ fontWeight: 800, fontSize: 13, color: "#1e293b" }}>WBManager Suite</div>
            <div style={{ fontSize: 9, color: "#94a3b8" }}>v13.0 All-in-One</div>
          </div>
        ...
        {/* WBM Nav */}
        <div className="nav-section">💬 WhatsApp Manager</div>
        ...
        {/* Schema Nav */}
        <div className="nav-section">🔖 Schema Manager</div>
        <nav style={{ padding: "0 8px", display: "flex", flexDirection: "column", gap: 2 }}>
        ...
        <div style={{ padding: "12px 14px", borderTop: "1px solid #e2e8f0" }}>
          ...
          <div style={{ fontSize: 11, color: "#16a34a", fontWeight: 600, marginBottom: 4 }}>💬 WA: {sites.filter(s => s.enabled).length} active · {sites.filter(s => s.payment === "paid").length} paid</div>
          <div style={{ fontSize: 11, color: "#7c3aed", fontWeight: 600, marginBottom: 10 }}>🔖 Schema: {schemaSites.filter(s => s.enabled).length} active · {schemaSites.filter(s => s.payment === "paid").length} paid</div>
      ...
      <div className={`main-wrap${isWide ? " with-sidebar" : ""}`}>
        <div className="topbar">
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 800, fontSize: 16, color: "#1e293b", display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
              {isSchemaModule ? "🔖 Schema Manager" : "💬 WhatsApp Manager"}
              <span className="live-badge">● LIVE</span>
              {!isSchemaModule && <span className="geo-badge">🌍 GEO</span>}
              {isSchemaModule && <span className="schema-badge">SEO</span>}
            </div>
            <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>
              {currentView === "dashboard" ? (isSchemaModule ? `${schemaSites.filter(s => s.enabled).length} active schemas` : `${sites.filter(s => s.enabled).length} active · Geo Routing ON`) : (isSchemaModule ? selSchema?.name : selWBM?.name) || "Naya website"}
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            {currentView === "dashboard" && <button className="btn-g" style={{ fontSize: 12, padding: "8px 12px" }} onClick={isSchemaModule ? loadSchema : loadWBM}>🔄</button>}
            {currentView === "dashboard"
              ? <button className={isSchemaModule ? "btn-schema" : "btn-p"} onClick={isSchemaModule ? openAddSchema : openAddWBM}>+ Add</button>
              : <button className="btn-g" onClick={() => isSchemaModule ? goSchema("dashboard") : goWBM("dashboard")}>← Back</button>}
          </div>
        </div>
        ...
        <div className="page">
          ...
          {/* ══ WBM DASHBOARD ══ */}
          {!isSchemaModule && viewWBM === "dashboard" && (<>
            ...
            <div style={{ background: "#fdf4ff", border: "1px solid #e9d5ff", borderRadius: 12, padding: "12px 16px", fontSize: 13, color: "#6b21a8" }}>🌍 <b>Geo Routing Active</b> — Customer ka country detect → usi country ka number use hoga</div>
            {loadingWBM ? <div className="loading"><div className="spinner" /><span style={{ color: "#94a3b8" }}>Loading...</span></div> : (
              <div className="card">
                <div className="card-title">💬 WhatsApp Sites ({sites.length})</div>
                {sites.length === 0 ? <div style={{ textAlign: "center", padding: 40 }}><div style={{ fontSize: 48, marginBottom: 14 }}>🌐</div><p style={{ color: "#94a3b8", marginBottom: 18 }}>Koi website nahi!</p><button className="btn-p" onClick={openAddWBM}>+ Add Website</button></div> : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    {sites.map(site => <SiteCard key={site.id} site={site} onScript={openScriptWBM} onEdit={openEditWBM} onDelete={s => setDelItem(s)} onToggle={toggleWBM} onPlan={planWBM} onPayment={paymentWBM} onVerify={verifyWBM} verifying={verifyingWBM} isSchema={false} />)}
                  </div>
                )}
              </div>
            )}
          ...
          {/* ══ WBM ADD/EDIT ══ */}
          {!isSchemaModule && (viewWBM === "add" || viewWBM === "edit") && (
            <div className="card">
              <div className="form-grid" style={{ marginBottom: 4 }}>
                <div className="fg"><label className="lbl">Website Name *</label><input className="inp" placeholder="My Online Store" value={formWBM.name} onChange={e => setFormWBM(f => ({ ...f, name: e.target.value }))} /></div>
                ...
                <div className="fg" style={{ gridColumn: "1/-1" }}><label className="lbl">Secret ID (optional)</label><input className="inp" placeholder="e.g. SDP001" value={formWBM.secretKey} onChange={e => setFormWBM(f => ({ ...f, secretKey: e.target.value }))} /></div>
              </div>
              <div className="fg">
                <label className="lbl">WhatsApp Numbers * <span style={{ color: "#7c3aed", fontSize: 11 }}>🌍 Geo auto routing</span></label>
                ...
                {formWBM.numbers.map((n, i) => {
                  const info = n.trim() ? getNumberInfo(n) : null;
                  return (
                    <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
                      <span style={{ fontSize: 18 }}>{info ? info.flag : "📱"}</span>
                      <input className="inp" style={{ flex: 1 }} placeholder="+92 300 1234567" value={n} onChange={e => { const a = [...formWBM.numbers]; a[i] = e.target.value; setFormWBM(f => ({ ...f, numbers: a })); }} />
                      {info && <span style={{ fontSize: 11, color: "#7c3aed", fontWeight: 600, whiteSpace: "nowrap" }}>{info.name}</span>}
                      {formWBM.numbers.length > 1 && <button onClick={() => setFormWBM(f => ({ ...f, numbers: f.numbers.filter((_, j) => j !== i) }))} style={{ background: "#fef2f2", border: "1px solid #fecaca", color: "#dc2626", borderRadius: 8, padding: "8px 11px", cursor: "pointer", fontWeight: 700 }}>✕</button>}
                    </div>
                  );
                })}
              ...
              <div className="fg">
                ...
                <div className="plan-compare">
                  <div className={`plan-card basic${formWBM.plan === "basic" ? " selected" : ""}`} onClick={() => setFormWBM(f => ({ ...f, plan: "basic" }))}>
                    <div className="plan-card-title">🔵 Basic</div><div className="plan-price">PKR 499<span style={{ fontSize: 12 }}>/mo</span></div>
                    <div className="plan-feature">WhatsApp button</div><div className="plan-feature">Inquiry message</div><div className="plan-feature">🌍 Geo Routing</div><div className="plan-feature no">Product details nahi</div>
                  </div>
                  <div className={`plan-card pro${formWBM.plan === "pro" ? " selected" : ""}`} onClick={() => setFormWBM(f => ({ ...f, plan: "pro" }))}>
                    <div className="plan-card-title">🚀 Pro</div><div className="plan-price">PKR 999<span style={{ fontSize: 12 }}>/mo</span></div>
                    <div className="plan-feature">WhatsApp button</div><div className="plan-feature">🌍 Geo Routing</div><div className="plan-feature">Product + Price auto</div><div className="plan-feature">Image + Link + Secret ID</div>
                  </div>
                </div>
              </div>
              <div className="card" style={{ background: "#f8fafc", boxShadow: "none" }}>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  <button className={`btn-pay${formWBM.payment === "paid" ? " paid" : ""}`} onClick={() => setFormWBM(f => ({ ...f, payment: "paid" }))}>✅ Paid</button>
                  <button className={`btn-pay${formWBM.payment === "pending" ? " pend" : ""}`} onClick={() => setFormWBM(f => ({ ...f, payment: "pending" }))}>⏳ Pending</button>
                </div>
              </div>
              <div style={{ display: "flex", gap: 10, marginTop: 20, justifyContent: "flex-end" }}>
                <button className="btn-g" onClick={() => goWBM("dashboard")}>Cancel</button>
                <button className="btn-p" onClick={saveWBM} disabled={saving}>{saving ? "Saving..." : viewWBM === "add" ? "Add Website" : "Save"}</button>
              </div>
            </div>
          )}

          {/* ══ WBM SCRIPT ══ */}
          {!isSchemaModule && viewWBM === "script" && selWBM && (
            <div className="card">
              ...
              {tabWBM === "script" && (<>
                ...
                <div className="success-box">⚡ Real-time + 🌍 Geo Routing — Client sirf ek baar paste kare!</div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
                  <div><div style={{ fontWeight: 700, fontSize: 16, color: "#1e293b" }}>{selWBM.enabled && selWBM.payment === "paid" ? "✅ Active" : "⛔ Inactive"} — {selWBM.plan === "pro" ? "🚀 Pro" : "🔵 Basic"}</div><div style={{ fontSize: 12, color: "#94a3b8", marginTop: 4 }}>Paste before &lt;/body&gt;</div></div>
          ...
          {isSchemaModule && viewSchema === "dashboard" && (<>
            ...
            <div className="schema-info">🔖 <b>Schema Markup Manager</b> — Client ek baar script paste kare → har page/post pe automatic Schema apply hoga. Naye posts pe bhi! Google rich results mein dikhega!</div>
            {loadingSchema ? <div className="loading"><div className={`spinner spinner-schema`} /><span style={{ color: "#94a3b8" }}>Loading...</span></div> : (
              <div className="card">
                <div className="card-title">🔖 Schema Sites ({schemaSites.length})</div>
                {schemaSites.length === 0 ? <div style={{ textAlign: "center", padding: 40 }}><div style={{ fontSize: 48, marginBottom: 14 }}>🔖</div><p style={{ color: "#94a3b8", marginBottom: 18 }}>Koi schema site nahi!</p><button className="btn-schema" onClick={openAddSchema}>+ Add Website</button></div> : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    {schemaSites.map(site => <SiteCard key={site.id} site={{ ...site, installedAt: site.createdAt }} onScript={openScriptSchema} onEdit={openEditSchema} onDelete={s => setDelItem({ ...s, _type: "schema" })} onToggle={toggleSchema} onPlan={planSchema} onPayment={paymentSchema} onVerify={verifySchema} verifying={verifyingSchema} isSchema={true} />)}
                  </div>
                )}
              </div>
            )}
          ...
          {isSchemaModule && (viewSchema === "add" || viewSchema === "edit") && (
            <div className="card">
              <div style={{ background: "#fdf4ff", border: "1px solid #e9d5ff", borderRadius: 10, padding: "12px 16px", fontSize: 13, color: "#6b21a8", marginBottom: 16 }}>
                🔖 <b>Schema Markup</b> — Yeh info Google mein rich results ke liye use hogi. Business info ek baar add karo — sab pages pe auto apply!
              </div>
              <div className="form-grid" style={{ marginBottom: 4 }}>
                <div className="fg"><label className="lbl">Client Name *</label><input className="inp inp-schema" placeholder="My Store" value={formSchema.name} onChange={e => setFormSchema(f => ({ ...f, name: e.target.value }))} /></div>
                <div className="fg"><label className="lbl">Website URL *</label><input className="inp inp-schema" placeholder="https://yoursite.com" value={formSchema.url} onChange={e => setFormSchema(f => ({ ...f, url: e.target.value }))} /></div>
                <div className="fg"><label className="lbl">Business Name</label><input className="inp inp-schema" placeholder="ShopEase Deals PK" value={formSchema.businessName} onChange={e => setFormSchema(f => ({ ...f, businessName: e.target.value }))} /></div>
                <div className="fg">
                  <label className="lbl">Business Type</label>
                  <select className="inp inp-schema" value={formSchema.businessType} onChange={e => setFormSchema(f => ({ ...f, businessType: e.target.value }))}>
                    {["Organization", "LocalBusiness", "Store", "OnlineStore", "ClothingStore", "ElectronicsStore", "FoodEstablishment", "Restaurant", "HealthAndBeautyBusiness"].map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>
                <div className="fg" style={{ gridColumn: "1/-1" }}><label className="lbl">Business Description</label><input className="inp inp-schema" placeholder="Best online store in Pakistan..." value={formSchema.businessDesc} onChange={e => setFormSchema(f => ({ ...f, businessDesc: e.target.value }))} /></div>
                <div className="fg"><label className="lbl">Phone</label><input className="inp inp-schema" placeholder="+92 309 4626298" value={formSchema.businessPhone} onChange={e => setFormSchema(f => ({ ...f, businessPhone: e.target.value }))} /></div>
                <div className="fg"><label className="lbl">Email</label><input className="inp inp-schema" placeholder="info@store.com" value={formSchema.businessEmail} onChange={e => setFormSchema(f => ({ ...f, businessEmail: e.target.value }))} /></div>
                <div className="fg" style={{ gridColumn: "1/-1" }}><label className="lbl">Address</label><input className="inp inp-schema" placeholder="Lahore, Pakistan" value={formSchema.businessAddress} onChange={e => setFormSchema(f => ({ ...f, businessAddress: e.target.value }))} /></div>
                <div className="fg" style={{ gridColumn: "1/-1" }}><label className="lbl">Logo URL</label><input className="inp inp-schema" placeholder="https://yoursite.com/logo.png" value={formSchema.businessLogo} onChange={e => setFormSchema(f => ({ ...f, businessLogo: e.target.value }))} /></div>
              </div>

              {/* Social Links */}
              <div className="fg">
                <label className="lbl">Social Media Links</label>
                {formSchema.socialLinks.map((link, i) => (
                  <div key={i} style={{ display: "flex", gap: 8, marginBottom: 6, alignItems: "center" }}>
                    <input className="inp inp-schema" style={{ flex: 1 }} value={link} onChange={e => { const a = [...formSchema.socialLinks]; a[i] = e.target.value; setFormSchema(f => ({ ...f, socialLinks: a })); }} />
                    <button onClick={() => setFormSchema(f => ({ ...f, socialLinks: f.socialLinks.filter((_, j) => j !== i) }))} style={{ background: "#fef2f2", border: "1px solid #fecaca", color: "#dc2626", borderRadius: 8, padding: "8px 11px", cursor: "pointer", fontWeight: 700 }}>✕</button>
                  </div>
                ))}
                <div style={{ display: "flex", gap: 8 }}>
                  <input className="inp inp-schema" style={{ flex: 1 }} placeholder="https://facebook.com/yourpage" value={socialInput} onChange={e => setSocialInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && socialInput.trim()) { setFormSchema(f => ({ ...f, socialLinks: [...f.socialLinks, socialInput.trim()] })); setSocialInput(""); } }} />
                  <button onClick={() => { if (socialInput.trim()) { setFormSchema(f => ({ ...f, socialLinks: [...f.socialLinks, socialInput.trim()] })); setSocialInput(""); } }} style={{ background: "#fdf4ff", border: "1px solid #e9d5ff", color: "#7c3aed", borderRadius: 8, padding: "8px 14px", cursor: "pointer", fontWeight: 700 }}>+ Add</button>
                </div>
              </div>
              ...
              <div className="fg">
                <label className="lbl">SELECT PLAN *</label>
                <div className="plan-compare">
                  <div className={`plan-card basic${formSchema.plan === "basic" ? " selected" : ""}`} onClick={() => setFormSchema(f => ({ ...f, plan: "basic" }))}>
                    <div className="plan-card-title">🔵 Basic</div><div className="plan-price">PKR 499<span style={{ fontSize: 12 }}>/mo</span></div>
                    <div className="plan-feature">Organization Schema</div><div className="plan-feature">Article Schema</div><div className="plan-feature">BreadcrumbList</div><div className="plan-feature">WebSite Schema</div><div className="plan-feature no">Product Schema nahi</div>
                  </div>
                  <div className={`plan-card pro${formSchema.plan === "pro" ? " selected" : ""}`} onClick={() => setFormSchema(f => ({ ...f, plan: "pro" }))}>
                    <div className="plan-card-title">🚀 Pro</div><div className="plan-price">PKR 999<span style={{ fontSize: 12 }}>/mo</span></div>
                    <div className="plan-feature">Sab Basic schemas</div><div className="plan-feature">Product Schema</div><div className="plan-feature">ItemList Schema</div><div className="plan-feature">Price auto detect</div><div className="plan-feature">Rich results in Google</div>
                  </div>
                </div>
              </div>
              ...
              <div className="card" style={{ background: "#f8fafc", boxShadow: "none" }}>
                <div className="card-title">💰 Payment</div>
                <div style={{ display: "flex", gap: 10 }}>
                  <button className={`btn-pay${formSchema.payment === "paid" ? " paid" : ""}`} onClick={() => setFormSchema(f => ({ ...f, payment: "paid" }))}>✅ Paid</button>
                  <button className={`btn-pay${formSchema.payment === "pending" ? " pend" : ""}`} onClick={() => setFormSchema(f => ({ ...f, payment: "pending" }))}>⏳ Pending</button>
                </div>
              </div>
              <div style={{ display: "flex", gap: 10, marginTop: 20, justifyContent: "flex-end" }}>
                <button className="btn-g" onClick={() => goSchema("dashboard")}>Cancel</button>
                <button className="btn-schema" onClick={saveSchema} disabled={saving}>{saving ? "Saving..." : viewSchema === "add" ? "Add Website" : "Save"}</button>
              </div>
            </div>
          )}
          ...
          {isSchemaModule && viewSchema === "script" && selSchema && (
            <div className="card">
              ...
              <div className="schema-info">
                🔖 <b>Schema Types jo apply honge:</b><br/>
                <div style={{ marginTop: 8 }}>
                  <span className="schema-type-badge">Organization</span>
                  <span className="schema-type-badge">WebSite</span>
                  <span className="schema-type-badge">Article</span>
                  <span className="schema-type-badge">BreadcrumbList</span>
                  {selSchema.plan === "pro" && <><span className="schema-type-badge">Product ⭐</span><span className="schema-type-badge">ItemList ⭐</span></>}
                </div>
              </div>
              <div className="success-box">
                ✅ <b>One-time paste</b> — Har page pe auto apply!<br/>
                ✅ Naye posts pe bhi automatically!<br/>
                ✅ Real-time enable/disable control!
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 16, color: "#1e293b" }}>{selSchema.enabled && selSchema.payment === "paid" ? "✅ Active" : "⛔ Inactive"} — {selSchema.plan === "pro" ? "🚀 Pro" : "🔵 Basic"}</div>
                  <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 4 }}>Paste before &lt;/body&gt; — sirf ek baar!</div>
                </div>
                <button className="btn-schema" style={copiedSchema ? { background: "#6d28d9" } : {}} onClick={() => { navigator.clipboard.writeText(genSchemaScript(selSchema)); setCopiedSchema(true); toast_("Schema script copied! ✅"); setTimeout(() => setCopiedSchema(false), 2500); }}>{copiedSchema ? "✓ Copied!" : "📋 Copy Script"}</button>
              </div>
              <pre className="script-box" style={{ color: "#c4b5fd" }}>{genSchemaScript(selSchema)}</pre>

              <div style={{ marginTop: 16, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 10, padding: "14px 16px" }}>
                <div style={{ fontWeight: 700, fontSize: 13, color: "#1e293b", marginBottom: 10 }}>⚙️ Quick Controls</div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  ...
                  <span style={{ fontWeight: 700, color: selSchema.enabled ? "#16a34a" : "#94a3b8", fontSize: 13 }}>{selSchema.enabled ? "✅ Schema ON" : "⏸ Schema OFF"}</span>
